...
// Remove header
    ...
    new Promise((resolve, reject) => {
        ...
        if (req.body.destination === 'perfil' ){
            path = './public/'+ req.body.user + '/images/'+ req.body.destination + '/' +  req.body.user + ' n0.png';
        }

        if (req.body.destination === 'DNI'){
            path = './public/'+ req.body.user + '/images/'+ req.body.destination + '/' + req.body.fileName +'.png';
        }

        else{
            path = './public/'+ req.body.user + '/images/'+ req.body.destination + '/' +  req.body.user + ' n'+fileNames.length+'.png';
        }
...
// Remove header
    ...
    new Promise((resolve, reject) => {
        let path = "./public/images/documents/" + req.body.fileName + '.' + req.body.fileExt;
        fs.writeFile(path, base64Image, {encoding: 'base64'}, function (err) {
            if (err) {
                log.info("Error: ", err);
                reject("Error: ", err);
