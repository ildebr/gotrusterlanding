...
import { Typography, FormControl, CircularProgress, InputAdornment, TextField, OutlinedInput, InputLabel } from '@material-ui/core';
...
class UserCheck extends Component {
  ...
  inputChange = (e) => {
    ...
    self.setState({
      ...
      typingTimeout: setTimeout(function () {
        if (!isEmpty(e.target.value) && (e.target.value.indexOf(" ") === -1) && (valida==='0') && (totalChar <= 20)) {
          self.setState({            
            evalua:false,
            textValido:''
          })
  ...
  render() {
    ...
    const tooltip = <Tooltip id="button-tooltip">
    ...
    return (
      <FormControl className={classes.boxCaptureControl} variant="outlined">
        ...
        <TextField id="userID" name="userID" label="Usuario" placeholder="Usuario" type="text" size="small" variant="outlined" className={ClassNames(classes.boxCaptureInput, { successInput: success })} onBlur={(e) => { this.inputChange(e) }}
          required />
