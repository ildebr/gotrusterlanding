...
function UsersTable(props) {
  ...
  const handleEnable = (data) => {
    confirm({
      title: '¿Estas Seguro?',
      description: `Esta seguro de habilitar a ${data.firstName} ${data.lastName}.`,
  ...
  const handleDisable = (data) => {
    confirm({
      title: '¿Estas Seguro?',
      description: `Esta seguro de deshabilitar a ${data.firstName} ${data.lastName}.`,
      cancellationText: 'Cancelar',
      confirmationText: 'Aceptar',
      confirmationButtonProps: { autoFocus: true },
    })
      ...
      <Table>
        <TableHead>
          <TableRow classes={{ root: classes.tableRow }}>
            ...
            <TableCell className={classes.tableHeader}>INGRESO</TableCell>
        ...
        <TableBody>
          {data?.map((row, index) => (
            <TableRow key={index} classes={{ root: classes.tableRow }}>
              ...
              <TableCell className={classes.tableCell}>
                {moment(row.createdDate).format('D/MM/YYYY')}
              </TableCell>
              <TableCell className={classes.tableCell}>{row.email}</TableCell>
              <TableCell className={classes.tableCell}>
                {moment(row.lastModifiedDate).format('D/MM/YYYY')}
              </TableCell>
              ...
              <TableCell className={classes.noBorderCell} />
              <TableCell className={classes.tableCellActions}>
                <AntSwitch onChange={(e) => enabler(e, row)} />
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
