...
import SelfieRow from '../../components/admin/selfieRow'
...
function AdminSelfie(props) {
            ...
            setRows(
              response.filter((row) => {
                if (row.validationDetailsDTOS?.length > 0) {
                  const validRow = row.validationDetailsDTOS.find(
                    (vd) =>
                      vd.validationName === 'SELFIE' &&
                      vd.validationStatus === 'PENDING'
                  )
                  const hasDniApproved = row.validationDetailsDTOS.find(
                    (vd) =>
                      vd.validationName === 'DNI' &&
                      vd.validationStatus === 'APPROVED'
                  )
                  if (validRow) {
  ...
  const handleApprove = async (data) => {
    ...
    valDetail.validationModificationDate = moment(new Date()).format(
      'YYYY-MM-DD'
    )
    try {
      const token = getToken()
      const data = await cliente
        .put(`${ValidatioDetail()}/${valDetail.id}`, valDetail, {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
            ...
            <Grid item container spacing={1} direction="row">
              ...
              <Grid item>
                <Typography className={classes.resultsText}>
                  Validaciones de Selfies Pendientes
                </Typography>
              </Grid>
            </Grid>
...
export default withStyles(styles, { withTheme: true })(AdminSelfie)
