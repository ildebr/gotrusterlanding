...
function Users(props) {
  ...
  const handleClearSearch = (ev) => {
    ev.target.value = ''
    setValueSearch('')
    setRows(JSON.parse(localStorage.getItem(ROWS_INDIVIDUAL)))
  }

  const handleFilter = (value, curSelectedSearch) => {
    setValueSearch(value)
    const rowsAux = JSON.parse(localStorage.getItem(ROWS_INDIVIDUAL))
    const newRows =
      value.length == 0
        ? rowsAux
        : rowsAux.filter((row) => {
            switch (curSelectedSearch ? curSelectedSearch : selectedSearch) {
              case 'NOMBRE':
                ...
                return false
              case 'TAGLINE':
                if (row.occupation?.toLowerCase().includes(value.toLowerCase()))
                  return true
                return false
              case 'CORREO':
                if (row.email?.toLowerCase().includes(value.toLowerCase()))
                  return true
                return false
              case 'FECHAMODIFICACION':
                if (
                  moment(row.lastModifiedDate)
                    .format('D/MM/YYYY')
                    ?.toLowerCase()
                    .includes(value.toLowerCase())
                )
      ...
      <Container maxWidth="lg">
        <Grid container maxWidth="lg" direction="column">
          ...
          >
            ...
            >
              <Grid item>
                <Typography className={classes.subTitle}>Usuarios</Typography>
              </Grid>
              <Grid item>
              ...
              <Grid item>
                <div className={classes.searchBar}>
                  ...
                  >
                    <StyledMenuItem
                      onClick={() => {
                        setSelectedSearch('NOMBRE')
                        handleFilter(valueSearch, 'NOMBRE')
                        handleClose()
                      }}
                    ...
                    <StyledMenuItem
                      onClick={() => {
                        setSelectedSearch('TAGLINE')
                        handleFilter(valueSearch, 'TAGLINE')
                        handleClose()
                      }}
                    ...
                    <StyledMenuItem
                      onClick={() => {
                        setSelectedSearch('CORREO')
                        handleFilter(valueSearch, 'CORREO')
                        handleClose()
                      }}
                    ...
                    <StyledMenuItem
                      onClick={() => {
                        setSelectedSearch('FECHAMODIFICACION')
                        handleFilter(valueSearch, 'FECHAMODIFICACION')
                        handleClose()
                      }}
