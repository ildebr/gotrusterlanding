...
class RegisterSecond extends Component {
  ...
  handleYearChange = (e) => {
    ...
    let today = new Date()
    let date = `${today.getFullYear() - 18}-${pad(
      today.getMonth() + 1,
      2
    )}-${pad(today.getDate(), 2)}`
    let birthDate = `${e.target.value}-${this.handleSetMonths(
      this.state.months
    )}-${this.state.day}`
  ...
  handleCreateAccountSubmit = (e) => {
    ...
    let birthDate = `${this.state.years}-${this.handleSetMonths(
      this.state.months
    )}-${this.state.day}`
    ...
    auth
      .login('administrator', 'Truster2021App')
      .then(() => {
        const token = getToken()
        console.log(token)
        cliente
          .post(AccountRegister(), dataRegister, {
            headers: {
          ...
          .then((response) => {
            ...
            cliente
              .post(AddressOperationsFull(), dataAdress, {
                headers: {
                  Accept: 'application/json',
                  'Content-Type': 'application/json',
                  Authorization: `Bearer ${token}`,
  ...
  render() {
    ...
    let $genderError = this.state.genderError
      ...
      : ''
    let $nationalityError = this.state.nationalityError
      ? '*Por favor selecciona una opción'
      : ''
    ...
    let $locationError = this.state.locationError
      ? '*Por favor selecciona una opción'
      : ''
    let $dayError = this.state.dayError
      ? '*Por favor selecciona una opción'
      : ''
    let $monthError = this.state.monthError
      ? '*Por favor selecciona una opción'
      : ''
    let $yearError = this.state.yearError
      ? '*Por favor selecciona una opción'
      : ''
    ...
    const isEnabled =
      ...
      this.state.day !== 'Dia' &&
      this.state.months !== 'Mes' &&
      this.state.years !== 'Año' &&
                  ...
                  <div className={classes.paper}>
                    <Box>
                      <Typography className={classes.paperWelcome}>
                        Estás a punto de tener tu cuenta Truster.{' '}
                      </Typography>
                    </Box>
                    {width > 550 ? (
                      <Grid justify="center" style={{ maxWidth: 400 }}>
                        ...
                        >
                          ...
                          <SelectBase
                            ...
                            style={{ paddingLeft: 20 }}
                          >
                            {this.state.municipality.map((locality) => (
                          ...
                          <Grid container item>
                            ...
                            <Button
                              ...
                              fullWidth
                              disabled={
                                this.state.enabledComponent || !isEnabled
                              }
                              className={
                                this.state.error
                                  ? classes.loginError
                          ...
                          <Grid container justify="flex-start">
                            ...
                            >
                              <SelectBase
                                ...
                                className={
                                  this.state.dayError
                                    ? classes.errorSelectMobile
                                    : classes.selectMobile
                                }
                                inputProps={{
                                  classes: {
                                    icon: classes.icon,
